	...
	public static function add_discount_types( $discount_types ) {
		...
		return array_merge(
			...
			array(
				...
				'sign_up_fee_percent' => __( 'Sign Up Fee % Discount', WC_Subscriptions::$text_domain ),
				'recurring_fee'       => __( 'Recurring Discount', WC_Subscriptions::$text_domain ),
				'recurring_percent'   => __( 'Recurring % Discount', WC_Subscriptions::$text_domain ),
			)
		);
	}
				...
				foreach ( $cart->applied_coupons as $code ) {
					...
					if ( $coupon->apply_before_tax() && $coupon->is_valid() ) {
						...
						if ( in_array( $calculation_type, array( 'combined_total', 'none' ) ) ) {
							...
							if ( ! WC_Subscriptions_Cart::cart_contains_free_trial() || ( 'none' === $calculation_type &&  WC_Subscriptions_Cart::cart_contains_free_trial() )  ) { // Apply recurring discounts to initial total
