	...
	public static function email_subject_new_renewal_order( $subject, $order ) {
		...
		if ( self::is_renewal( $order ) ) {
			...
			$subject  = apply_filters(
				...
				sprintf( __( '[%s] New Subscription Renewal Order (%s)', WC_Subscriptions::$text_domain ), $blogname, $order->get_order_number() ),
				$order
			);
		}
	...
	public static function email_subject_customer_procesing_renewal_order( $subject, $order ) {
		...
		if ( self::is_renewal( $order ) ) {
			...
			$subject  = apply_filters(
				...
				sprintf( __( '[%s] Subscription Renewal Order', WC_Subscriptions::$text_domain ), $blogname ),
				$order
			);
		}
