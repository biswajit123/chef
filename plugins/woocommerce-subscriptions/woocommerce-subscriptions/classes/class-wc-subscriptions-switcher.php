...
class WC_Subscriptions_Switcher {
	...
	public static function init(){
		...
		// Make sure the first payment date for a new subscription takes is correct
		add_filter( 'woocommerce_subscription_next_payment_date', __CLASS__ . '::get_first_payment_date', 11, 4 );
	...
	public static function add_settings( $settings ) {
		...
		array_splice( $settings, 12, 0, array(
			...
			array(
				'name'     => __( 'Switching', WC_Subscriptions::$text_domain ),
				'type'     => 'title',
				'desc'     => sprintf( __( 'Allow subscribers to switch (upgrade or downgrade) between different subscriptions. %sLearn more%s.', WC_Subscriptions::$text_domain ), '<a href="' . esc_url( 'http://docs.woothemes.com/document/subscription-switching-guide/' ) . '">', '</a>' ),
				'id'       => WC_Subscriptions_Admin::$option_prefix . '_switch_settings'
			),

			array(
				'name'    => __( 'Allow Switching', WC_Subscriptions::$text_domain ),
				'desc'    => __( 'Allow subscribers to switch between subscriptions combined in a grouped product, different variations of a Variable subscription or don\'t allow switching.', WC_Subscriptions::$text_domain ),
				'tip'     => '',
				...
				'type'    => 'select',
				'options' => array(
					'no'               => __( 'Never', WC_Subscriptions::$text_domain ),
					'variable'         => __( 'Between Subscription Variations', WC_Subscriptions::$text_domain ),
					'grouped'          => __( 'Between Grouped Subscriptions', WC_Subscriptions::$text_domain ),
					'variable_grouped' => __( 'Between Both Variations & Grouped Subscriptions', WC_Subscriptions::$text_domain ),
				),
				'desc_tip' => true,
			),

			array(
				'name'    => __( 'Apportion Recurring Payments', WC_Subscriptions::$text_domain ),
				'desc'    => __( 'When switching to a subscription with a different recurring payment or billing period, should the price paid for the existing billing period be apportioned when switching to the new subscription?', WC_Subscriptions::$text_domain ),
				'tip'     => '',
				...
				'options' => array(
					'no'                 => __( 'Never', WC_Subscriptions::$text_domain ),
					'virtual'            => __( 'For Virtual Subscription Products Only', WC_Subscriptions::$text_domain ),
					'yes'                => __( 'For All Subscription Products', WC_Subscriptions::$text_domain ),
				),
				'desc_tip' => true,
			),

			array(
				'name'    => __( 'Apportion Sign up Fee', WC_Subscriptions::$text_domain ),
				'desc'    => __( 'When switching to a subscription with a sign up fee, you can require the customer pay only the gap between the existing subscription\'s sign up fee and the new subscription\'s sign up fee (if any).', WC_Subscriptions::$text_domain ),
				'tip'     => '',
				...
				'type'    => 'select',
				'options' => array(
					'no'                 => __( 'Never (do not charge a sign up fee)', WC_Subscriptions::$text_domain ),
					'full'               => __( 'Never (charge the full sign up fee)', WC_Subscriptions::$text_domain ),
					'yes'                => __( 'Always', WC_Subscriptions::$text_domain ),
				),
				'desc_tip' => true,
			),

			array(
				'name'    => __( 'Apportion Subscription Length', WC_Subscriptions::$text_domain ),
				'desc'    => __( 'When switching to a subscription with a length, you can take into account the payments already completed by the customer when determining how many payments the subscriber needs to make for the new subscription.', WC_Subscriptions::$text_domain ),
				'tip'     => '',
			...
			array(
				'name'     => __( 'Switch Button Text', WC_Subscriptions::$text_domain ),
				'desc'     => __( 'Customise the text displayed on the button next to the subscription on the subscriber\'s account page. The default is "Switch Subscription", but you may wish to change this to "Upgrade" or "Change Subscription".', WC_Subscriptions::$text_domain ),
				'tip'      => '',
				'id'       => WC_Subscriptions_Admin::$option_prefix . '_switch_button_text',
				'css'      => 'min-width:150px;',
				'default'  => __( 'Switch Subscription', WC_Subscriptions::$text_domain ),
				'type'     => 'text',
				'desc_tip' => true,
			),
		...
		foreach ( $all_actions as $subscription_key => $actions ) {
			...
			if ( WC_Subscriptions_Manager::can_subscription_be_changed_to( 'new-subscription', $subscription_key, $user_id ) ) {
				$all_actions[$subscription_key] = array( 'switch' => array(
					...
					'name' => get_option( WC_Subscriptions_Admin::$option_prefix . '_switch_button_text', __( 'Switch Subscription', WC_Subscriptions::$text_domain ) )
					)
	...
	public static function switch_order_meta_box_section( $order ) {
		...
		if ( ! empty( $original_subscription_key ) ) {
			...
			printf(
				...
				__( 'Subscription Switched from Order:', WC_Subscriptions::$text_domain ),
		...
		if ( ! empty( $new_order_id ) ) {
			...
			printf(
				'<p>%1$s <a href="%2$s">%3$s</a></p>',
				__( 'Subscription Switched in Order:', WC_Subscriptions::$text_domain ),
	...
	public static function add_switched_status_string( $status_string ) {
		...
		if ( 'switched' === strtolower( $status_string ) )
			$status_string = __( 'Switched', WC_Subscriptions::$text_domain );
	...
	public static function validate_switch_request( $is_valid, $product_id, $quantity, $variation_id = '' ){
		...
		if ( $product_id == $subscription['product_id'] && ( empty( $variation_id ) || $variation_id == $subscription['variation_id'] ) ) {
			$woocommerce->add_error( __( 'You can not switch to the same subscription.', WC_Subscriptions::$text_domain ) );
			$is_valid = false;
		}
	...
	public static function maybe_set_apporitioned_totals( $total ) {
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( 'yes' == $apportion_sign_up_fee ) {
				...
				$subscription = WC_Subscriptions_Manager::get_subscription( $old_subscription_key );

				$sign_up_fee_due  = $product->subscription_sign_up_fee;

				if ( self::order_contains_subscription_switch( $subscription['order_id'] ) ) {
					$order = new WC_Order( $subscription['order_id'] );
					$sign_up_fee_paid = $order->get_total();
				} else {
					$sign_up_fee_paid = WC_Subscriptions_Order::get_item_sign_up_fee( $subscription['order_id'], $subscription['product_id'] );
				}
			...
			if ( 'yes' == $apportion_recurring_price || ( 'virtual' == $apportion_recurring_price && $is_virtual_product ) ) {
				...
				$old_order = new WC_Order( $subscription['order_id'] );
	...
	public static function get_first_payment_date( $next_payment_date, $subscription_key, $user_id, $type ) {
		...
		if ( self::order_contains_subscription_switch( $subscription['order_id'] ) && 0 == WC_Subscriptions_Manager::get_subscriptions_completed_payment_count( $subscription_key ) ) {
			...
			if ( 0 != $first_payment_timestamp ) {
				$next_payment_date = ( 'mysql' == $type && 0 != $first_payment_timestamp ) ? date( 'Y-m-d H:i:s', $first_payment_timestamp ) : $first_payment_timestamp;
			}
		}
	...
	public static function customise_subscription_price_string( $subscription_string ) {
		...
		if ( false !== $switch_details && 0 != $switch_details['first_payment_timestamp'] )
			$subscription_string = sprintf( __( '%s %s(next payment %s)%s', WC_Subscriptions::$text_domain ), $subscription_string, '<small>', date_i18n( woocommerce_date_format(), $switch_details['first_payment_timestamp'] ), '</small>' );
