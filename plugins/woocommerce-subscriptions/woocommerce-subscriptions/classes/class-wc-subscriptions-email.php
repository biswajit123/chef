...
class WC_Subscriptions_Email {
	...
	public static function add_emails( $email_classes ) {
		...
		require( 'emails/class-wcs-email-new-renewal-order.php' );
		require( 'emails/class-wcs-email-customer-processing-renewal-order.php' );
		require( 'emails/class-wcs-email-customer-completed-renewal-order.php' );
		require( 'emails/class-wcs-email-customer-renewal-invoice.php' );
		require( 'emails/class-wcs-email-cancelled-subscription.php' );
	...
	public static function hook_transactional_emails() {
		...
		$subscription_email_actions = array( 'cancelled_subscription', 'switched_subscription' );

		foreach ( $subscription_email_actions as $action )
			add_action( $action, __CLASS__ . '::send_subscription_email', 10, 2 );

		$default_email_actions = array(
		...
		foreach ( $default_email_actions as $action ) {
