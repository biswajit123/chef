		...
		foreach ( $woocommerce->payment_gateways->payment_gateways as $key => $gateway ) {
			...
			$woocommerce->payment_gateways->payment_gateways[$key]->form_fields += array(
				...
				'api_credentials' => array(
					...
					'type'        => 'title', 
					'description' => sprintf( __( 'Enter your PayPal API credentials to unlock subscription suspension and cancellation features. %sLearn More &raquo;%s', WC_Subscriptions::$text_domain ), '<a href="http://docs.woothemes.com/document/store-manager-guide/#section-4" target="_blank" tabindex="-1">', '</a>' ),
				),
