...
class WC_Subscriptions_Manager {
	...
	public static function get_subscriptions_completed_payment_count( $subscription_key ) {
		...
		return apply_filters( 'woocommerce_subscription_failed_payment_count', $completed_payments, $subscription_key );
	}
	...
	public static function get_all_users_subscriptions() {
		...
		foreach ( $user_ids as $user_id )
			$subscriptions[$user_id] = self::get_users_subscriptions( $user_id );
	...
	public static function update_next_payment_date( $new_payment_date, $subscription_key, $user_id = '', $timezone = 'server' ) {
		...
		if ( 0 == $next_payment_timestamp ) {
			...
			$response = new WP_Error( 'invalid-date', sprintf( __( 'Please enter a date after the trial period ends (%s).', WC_Subscriptions::$text_domain ), date_i18n( woocommerce_date_format(), $trial_expiration_timestamp + ( get_option( 'gmt_offset' ) * 3600 ) ) ) );
