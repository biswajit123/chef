...
class WC_Subscriptions_Change_Payment_Gateway {
		...
		foreach ( $all_actions as $subscription_key => $actions ) {
			...
			if ( WC_Subscriptions_Manager::can_subscription_be_changed_to( 'new-payment-method', $subscription_key, get_current_user_id() ) ) {
				...
				$all_actions[$subscription_key] = array( 'change_payment_method' => array(
					'url'  => $url,
					'name' => __( 'Change Payment Method', WC_Subscriptions::$text_domain ),
					)
	...
	public static function change_payment_method_via_pay_shortcode() {
		...
		if ( isset( $_POST['woocommerce_change_payment'] ) && $woocommerce->verify_nonce( 'change_payment' ) ) {
			...
			if ( $order->id == $order_id && $order->order_key == $order_key ) {
				...
				if ( $woocommerce->error_count() == 0 ) {
					...
					if ( $result['result'] == 'success' ) {
						$woocommerce->add_message( __( 'Payment method updated.', WC_Subscriptions::$text_domain ) );
	...
	private static function update_recurring_payment_method( $subscription_key, $order, $new_payment_method ) {
		...
		// Log change on order
		$order->add_order_note( sprintf( __( 'Recurring payment method changed from "%s" to "%s" by the subscriber from their account page.', WC_Subscriptions::$text_domain ), $old_payment_method_title, $new_payment_method_title ) );
