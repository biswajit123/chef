		...
		$order_ids = $wpdb->get_col(
			"SELECT `order_id` FROM $order_items_table
			 WHERE `order_item_id` IN (
				SELECT `order_item_id` FROM $order_item_meta_table
				WHERE `meta_key` = '_subscription_trial_length'
				AND `meta_value` > 0
				AND `order_item_id` NOT IN (
					SELECT `order_item_id` FROM $order_item_meta_table
					WHERE `meta_key` = '_subscription_sign_up_fee'
					AND `meta_value` > 0
				)
			)"
		);
...
<p><?php _e( 'The WooCommerce Subscriptions plugin has been updated!', WC_Subscriptions::$text_domain ); ?></p>
<p><?php _e( 'Before we send you on your way, we need to update your database to the newest version. If you do not have a recent backup of your site, now is a good time to create one.', WC_Subscriptions::$text_domain ); ?></p>
...
<?php
	...
	public static function upgrade_in_progress_notice() {
		...
		error_log( '$upgrade_transient_timeout = ' . print_r( $upgrade_transient_timeout, true ) );

		$time_until_update_allowed = $upgrade_transient_timeout - time();

		error_log( '$time_until_update_allowed = ' . print_r( $time_until_update_allowed, true ) );
...
<head>
	<meta http-equiv="Content-Type" content="<?php bloginfo( 'html_type' ); ?>; charset=<?php echo get_option( 'blog_charset' ); ?>" />
	<title><?php _e( 'WooCommerce Subscriptions Update in Progress', WC_Subscriptions::$text_domain ); ?></title>
	<?php wp_admin_css( 'install', true ); ?>
	<?php wp_admin_css( 'ie', true ); ?>
</head>
<body class="wp-core-ui">
<h1 id="logo"><img alt="WordPress" src="<?php echo plugins_url( 'images/woocommerce_subscriptions_logo.png', WC_Subscriptions::$plugin_file ); ?>" /></h1>
<h2><?php _e( 'The Upgrade is in Progress', WC_Subscriptions::$text_domain ); ?></h2>
<p><?php _e( 'The WooCommerce Subscriptions plugin is currently running its database upgrade routine.', WC_Subscriptions::$text_domain ); ?></p>
<p><?php printf( __( 'If you received a server error and reloaded the page to find this notice, please refresh the page in %s seconds and the upgrade routine will recommence without issues. Subscriptions can update approximately %s subscriptions before exhausting the memory available on your PHP installation (which has %s allocated). So depending on the number of subscriptions, you may need to restart the process a number of times.', WC_Subscriptions::$text_domain ), $time_until_update_allowed, $subscription_before_exhuastion, $memory_limit ); ?></p>
<p><?php _e( 'Rest assured, although the update process may take a little while, it is coded to prevent defects, your site is safe and will be up and running again, faster than ever, shortly.', WC_Subscriptions::$text_domain ); ?></p>
</body>
</html>
<?php
	die();
