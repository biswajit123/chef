...
class WCS_Email_Cancelled_Subscription extends WC_Email {
	...
	function __construct() {
		...
		$this->title       = __( 'Cancelled Subscription', WC_Subscriptions::$text_domain );
		$this->description = __( 'Cancelled Subscription emails are sent when a customer\'s subscription is cancelled (either by a store manager, or the customer).', WC_Subscriptions::$text_domain );

		$this->heading     = __( 'Subscription Cancelled', WC_Subscriptions::$text_domain );
		$this->subject     = __( '[{blogname}] Subscription Cancelled', WC_Subscriptions::$text_domain );
	...
	function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				'title' 		=> __( 'Enable/Disable', WC_Subscriptions::$text_domain ),
				'type' 			=> 'checkbox',
				'label' 		=> __( 'Enable this email notification', WC_Subscriptions::$text_domain ),
				'default' 		=> 'no'
			),
			'recipient' => array(
				'title' 		=> __( 'Recipient(s)', WC_Subscriptions::$text_domain ),
				'type' 			=> 'text',
				'description' 	=> sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to <code>%s</code>.', WC_Subscriptions::$text_domain ), esc_attr( get_option( 'admin_email' ) ) ),
				'placeholder' 	=> '',
				'default' 		=> ''
			),
			'subject' => array(
				'title' 		=> __( 'Subject', WC_Subscriptions::$text_domain ),
				'type' 			=> 'text',
				'description' 	=> sprintf( __( 'This controls the email subject line. Leave blank to use the default subject: <code>%s</code>.', WC_Subscriptions::$text_domain ), $this->subject ),
				'placeholder' 	=> '',
				'default' 		=> ''
			),
			'heading' => array(
				'title' 		=> __( 'Email Heading', WC_Subscriptions::$text_domain ),
				'type' 			=> 'text',
				'description' 	=> sprintf( __( 'This controls the main heading contained within the email notification. Leave blank to use the default heading: <code>%s</code>.', WC_Subscriptions::$text_domain ), $this->heading ),
				'placeholder' 	=> '',
				'default' 		=> ''
			),
			'email_type' => array(
				'title' 		=> __( 'Email type', WC_Subscriptions::$text_domain ),
				'type' 			=> 'select',
				'description' 	=> __( 'Choose which format of email to send.', WC_Subscriptions::$text_domain ),
				'default' 		=> 'html',
				'class'			=> 'email_type',
				'options'		=> array(
					'plain'		 	=> __( 'Plain text', WC_Subscriptions::$text_domain ),
					'html' 			=> __( 'HTML', WC_Subscriptions::$text_domain ),
					'multipart' 	=> __( 'Multipart', WC_Subscriptions::$text_domain ),
				)
			)
		);
    }
