...
class WCS_Email_New_Renewal_Order extends WC_Email_New_Order {
	...
	function __construct() {
		...
		$this->title          = __( 'New Renewal Order', WC_Subscriptions::$text_domain );
		$this->description    = __( 'New renewal order emails are sent when a subscription renewal payment is processed.', WC_Subscriptions::$text_domain );

		$this->heading        = __( 'New subscription renewal order', WC_Subscriptions::$text_domain );
		$this->subject        = __( '[{blogname}] New subscription renewal order ({order_number}) - {order_date}', WC_Subscriptions::$text_domain );
