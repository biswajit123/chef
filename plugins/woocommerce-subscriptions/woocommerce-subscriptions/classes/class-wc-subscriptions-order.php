...
class WC_Subscriptions_Order {
	...
	public static function pre_process_shop_order_meta( $post_id, $post ) {
		...
		if ( isset( $_POST['_recurring_payment_method'] ) ) {
			$chosen_payment_method   = stripslashes( $_POST['_recurring_payment_method'] );
			...
			if ( $chosen_payment_method != $existing_payment_method || empty( $chosen_payment_method ) ) {
				...
				foreach ( $product_ids as $product_id ) {
					if ( isset( $payment_gateways[$existing_payment_method] ) && $payment_gateways[$existing_payment_method]->supports( 'subscriptions' ) ) {
	...
	public static function prefill_order_item_meta( $item, $item_id ) {
		...
		if ( $item_id && WC_Subscriptions_Product::is_subscription( $product_id ) ) {
			...
			WC_Subscriptions_Manager::create_pending_subscription_for_order( $order_id, $product_id );
...
<div id="recurring_order_totals"<?php if ( ! $contains_subscription ) echo $display_none; ?>>
	...
	<div class="totals_group">
		<h4><?php _e( 'Shipping for Renewal Orders', WC_Subscriptions::$text_domain ); ?></h4>
		<ul class="totals">
			<li class="wide">
				<label><?php _e( 'Label:', WC_Subscriptions::$text_domain ); ?></label>
				<input type="text" id="_recurring_shipping_method_title" name="_recurring_shipping_method_title" placeholder="<?php _e( 'The shipping title for renewal orders', WC_Subscriptions::$text_domain ); ?>" value="<?php echo $order->recurring_shipping_method_title; ?>" class="first" />
			</li>

			<li class="left">
				<label><?php _e( 'Cost:', WC_Subscriptions::$text_domain ); ?></label>
				<input type="number" step="any" min="0" id="_order_recurring_shipping_total" name="_order_recurring_shipping_total" placeholder="0.00 <?php _e( '(ex. tax)', WC_Subscriptions::$text_domain ); ?>" value="<?php echo self::get_recurring_shipping_total( $order ); ?>" class="first" />
			</li>

			<li class="right">
				<label><?php _e( 'Method:', WC_Subscriptions::$text_domain ); ?></label>
				<select name="_recurring_shipping_method" id="_recurring_shipping_method" class="first">
					<option value=""><?php _e( 'N/A', WC_Subscriptions::$text_domain ); ?></option>
					<?php
						...
						if ( ! $found_method && ! empty( $chosen_shipping_method ) ) {
							echo '<option value="' . esc_attr( $chosen_shipping_method ) . '" selected="selected">' . __( 'Other', WC_Subscriptions::$text_domain ) . '</option>';
						} else {
							echo '<option value="other">' . __( 'Other', WC_Subscriptions::$text_domain ) . '</option>';
						}
					?>
				</select>
			</li>
	...
	<div class="totals_group">
		<h4><span class="tax_total_display inline_total"></span><?php _e( 'Tax Totals', WC_Subscriptions::$text_domain ); ?></h4>
		<ul class="totals">
	...
	<div class="totals_group">
		...
		<ul class="totals">
			...
			<li class="right" style="font-size: 1em;">
				<div style="font-weight: normal; color: #333;"><?php _e( 'Payment Method:', WC_Subscriptions::$text_domain ); ?></div>
				<div style="padding-top: 7px; font-style: italic;"><?php echo ( $manual_renewal ) ? __( 'Manual', WC_Subscriptions::$text_domain ) : $order->recurring_payment_method_title; ?></div>
			</li>
