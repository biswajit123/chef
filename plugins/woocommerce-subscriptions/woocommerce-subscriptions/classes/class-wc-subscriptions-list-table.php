...
class WC_Subscriptions_List_Table extends WP_List_Table {
	...
	public function get_sortable_columns() {
		...
		$sortable_columns = array(
			'status'              => array( '_subscription_status', false ),
			'order_id'            => array( 'order_id', false ),
			'title'               => array( '_order_item_name', false ),
			'user'                => array( 'user_display_name', false ),
			'start_date'          => array( '_subscription_start_date', true ),
			'expiry_date'         => array( '_subscription_expiry_date', false ),
			'trial_expiry_date'   => array( '_subscription_trial_expiry_date', false ),
			'end_date'            => array( '_subscription_end_date', false ),
...
<select id="dropdown_products_and_variations" name="_product_id" data-placeholder="<?php _e( 'Search for a product&hellip;', WC_Subscriptions::$text_domain ); ?>" style="width: 240px">
	<option value=""><?php _e( 'Show all products', WC_Subscriptions::$text_domain ) ?></option>
...
<select id="dropdown_customers" name="_customer_user">
	<option value=""><?php _e( 'Show all customers', WC_Subscriptions::$text_domain ) ?></option>
...
<p><?php printf( __( '%s has no subscription to %s%s%s (#%s) with the status "%s".', WC_Subscriptions::$text_domain ), $user->display_name, '<em>', $product->get_title(), '</em>', intval( $_GET['_product_id'] ), $_GET['status'] ); ?></p>
				<?php else : ?>
...
<p><?php printf( __( 'There are no subscriptions to %s%s%s (#%s) with the status "%s".', WC_Subscriptions::$text_domain ), '<em>', $product->get_title(), '</em>', intval( $_GET['_product_id'] ), $_GET['status'] ); ?></p>
		<?php else : ?>
...
<p><?php printf( __( '%sLearn more about managing subscriptions%s', WC_Subscriptions::$text_domain ), '<a href="http://docs.woothemes.com/document/store-manager-guide/#section-3" target="_blank">', ' &raquo;</a>' ); ?></p>
