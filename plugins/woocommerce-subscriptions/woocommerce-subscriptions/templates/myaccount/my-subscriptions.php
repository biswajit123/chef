...
<table class="shop_table my_account_subscriptions my_account_orders">
	...
	<thead>
		<tr>
			...
			<th class="subscription-end"><span class="nobr"><?php _e( 'End Date', WC_Subscriptions::$text_domain ); ?></span></th>
			<th class="subscription-actions"><span class="nobr"><?php _e( 'Actions', WC_Subscriptions::$text_domain ); ?></th>
		</tr>
	</thead>
		...
		<tr class="order">
			...
			<td class="subscription-next-payment">
				...
				<time title="<?php echo esc_attr( $next_payment_timestamp ); ?>">
					...
					<?php $payment_method_to_display = sprintf( __( 'Via %s', WC_Subscriptions::$text_domain ), $order->recurring_payment_method_title ); ?>
				...
				<?php else : ?>
					...
					<?php if ( absint( $time_diff ) > 0 && absint( $time_diff ) < 7 * 24 * 60 * 60 ) : ?>
						<?php if ( $time_diff > 0 ) : // In the future ?>
							<?php $expiry = sprintf( __( 'In %s', WC_Subscriptions::$text_domain ), human_time_diff( $end_timestamp ) ); ?>
						<?php else : // In the past ?>
							<?php $expiry = sprintf( __( '%s ago', WC_Subscriptions::$text_domain ), human_time_diff( $end_timestamp ) ); ?>
						<?php endif; ?>
					<?php else : ?>
...
<?php else : ?>
	...
	<p><?php printf( __( 'You have no active subscriptions. Find your first subscription in the %sstore%s.', WC_Subscriptions::$text_domain ), '<a href="' . get_permalink( woocommerce_get_page_id( 'myaccount' ) ) . '">', '</a>' ); ?></p>
