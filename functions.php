...
// Input. You can fill out any combination of the 3 values (except city only) and leave any blank
   ...
   if( !validate_zipCode( $_POST['billing_postcode'] ) ) :
...
function upd_subscription_of_current_holder(){
    ...
    /*update_post_meta($orderID, '_prev_order_recurring_total', $recurringBilling);*/
    update_post_meta($orderID, '_order_recurring_total', $deductBalance);       // update the subscription
    /*update_post_meta($orderID, '_order_total', $deductBalance);                 // update the subscription*/
    /*$order_item = $wpdb->get_row("SELECT order_item_id FROM wp_woocommerce_order_items WHERE order_id = ".$orderID);
      $order_item_id  = $order_item->order_item_id;
      woocommerce_update_order_item_meta($order_item_id, '_line_subtotal',$deductBalance);
      woocommerce_update_order_item_meta($order_item_id, '_line_total',$deductBalance);
      woocommerce_update_order_item_meta($order_item_id, '_subscription_recurring_amount',$deductBalance);
      woocommerce_update_order_item_meta($order_item_id, '_recurring_line_total',$deductBalance);
      woocommerce_update_order_item_meta($order_item_id, '_recurring_line_subtotal',$deductBalance);*/
    /* Update wallet */
...
function checkout_non_login_css(){

    if(!is_user_logged_in() && strpos($_SERVER['REQUEST_URI'], 'checkout')){
        ?>
        <style>
            #main #content {
                width: 100% !important;
...
add_action('wp_head','checkout_non_login_css');
    ...
    for($i=0; $i<5; $i++){
        if(!empty($d['ReferrerEmailForm']['email_'.$i])){
            ...
            $message = 'Hello '. $name.',<br /> <br /> 
				I just signed up for an interesting new service here in Gainesville which delivers everything you need to cook delicious meals at home. Basically, they deliver 3 delicious recipes each week along with all the fresh ingredients needed to make them.
				It seems like a great service seeing as I won\'t have to go shopping or do any meal planning :) Yeah!  
				<br /> <br /> 
				So, after I signed up they gave me a coupon code to share with friends which is good for <b>$20 off your first order</b>. I thought you could use the extra time each week, I know I sure could. 
				<br /> <br /> 
				Anyway, if your interested just use the code <b>'.$d['ref_code_holder'].'</b> on their website here: www.chefami.com
				<br /> <br /> 
				Talk soon!';
...
register_activation_hook( __FILE__, 'payment_date_scheduler' );
/**
 * On activation, set a time, frequency and name of an action hook to be scheduled.
 */
function payment_date_scheduler() {
    wp_schedule_event( current_time ( 'timestamp' ), 'hourly', 'payment_date_scheduler_hook' );
}
...
function prefix_do_this_hourly() {
    global $wpdb;
    if(date('D') == "Sun") {        // Cron run daily but execute only in sunday
...
function my_woocommerce_add_error( $error ) {
    if( 'Coupon does not exist!' == $error ) {
        $error = 'Invalid Code. Please note referral codes may only be applied to new subscriptions and are not valid for Single Deliveries.';
    }
    return $error;
}
